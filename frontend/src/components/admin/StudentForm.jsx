...
const StudentForm = ({ onSuccess, onCancel }) => {
  ...
  useEffect(() => {
    const fetchRollPreview = async () => {
      ...
      try {
        const response = await axios.post(
          'http://localhost:5000/api/students/preview-roll-number',
          {
        ...
        if (response.data && response.data.success) {
          setPreviewRoll(response.data.data.rollNumber);
        } else {
          setPreviewError('Failed to fetch preview.');
        }
      } catch (err) {
        console.error('Error fetching roll number preview:', err);
        setPreviewError(err.response?.data?.message || 'Error fetching preview.');
      } finally {
    ...
    fetchRollPreview();
  }, [formData.department, formData.batch, token]);
      ...
      <div>
        ...
        <div className="relative">
          ...
          <div className="relative">
            ...
            <input
              ...
              placeholder={
                isFetchingPreview
                  ? 'Loading preview...'
                  : 'Select department & batch to preview'
              }
              className="input-field pl-9 bg-gray-50 text-gray-700 cursor-not-allowed select-none"
              aria-readonly="true"
          ...
          {previewError ? (
            ...
            <p className="mt-1.5 text-xs text-indigo-600 flex items-center gap-1 font-medium">
              <span
                className="inline-block h-2 w-2 rounded-full bg-indigo-500 animate-pulse"
                aria-hidden="true"
              />
              Preview: Final roll number assigned on save
            </p>
          ) : (
